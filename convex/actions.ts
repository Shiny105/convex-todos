    ...
    handler: async (ctx, args) => {
        ...
        const response = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "Generate 3 a list of to-dos based on the given prompt. Please include a title and description. Please return a JSON object in the following format: { todos : [{ title: string, description: string}] }"
                },
                {
                    role: "user",
